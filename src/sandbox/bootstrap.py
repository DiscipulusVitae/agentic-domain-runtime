...
def run_bootstrap_state(
    ...
    if init:
        ...
        if state_path.exists() and not overwrite and not dry_run:
            if json_mode:
                print(json.dumps({
                ...
                print(f"Ошибка: Файл состояния уже существует по пути '{state_path}'. Используйте --overwrite для перезаписи.", file=sys.stderr)
            return 1
